...
# ─── module-level Cortex Analyst caller ──────────────────────────────────────
def call_analyst(payload: dict) -> dict:
    """Send payload to Cortex Analyst API and return parsed JSON."""
    start = time.time()
    logger.info("=== CORTEX ANALYST REQUEST ===")
    logger.info("URL: %s", ANALYST_ENDPOINT)
    ...
    logger.info("=== CORTEX ANALYST RESPONSE ===")
    ...
    logger.info("Analyst call (%.1fs): got %d blocks",
                elapsed, len(data["message"]["content"]))
    return data
        ...
        "messages": [
            {
              ...
              "content": [
                {"type":"text","text":
                  "Rewrite the question in plain language (1 sentence), then answer in 1-2 sentences using ONLY the SQL result. Highlight key insights."},
...
def run_agent(question: str, customer: str) -> Dict[str, Any]:
    ...
    if re.search(r"\b(count|list|how many|average)\b", question, flags=re.I) \
       or re.search(r"\bshow\s+(?:me|the|\d+)", question, flags=re.I):
        ...
        if sql_blocks:
            ...
            return {"answer": answer_table, "sql": sql_stmt,
    ...
    free_text = "\n\n".join(b["text"] for b in texts).strip() or "❓ no text returned"
